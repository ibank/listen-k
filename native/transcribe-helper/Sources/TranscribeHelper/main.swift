...
struct TranscribeHelper {
    ...
    actor StreamController {
        ...
        // Last time stop() completed, used to enforce a minimum cooldown
        // before the next start() so the detached stopStreamTranscription
        // can finish tearing the shared AudioProcessor down. Without this,
        // a rapid toggle (especially on first launch, when the Core ML
        // kernels are still JIT-compiling) races the previous stream's
        // teardown against the next stream's setup and the AVAudioEngine
        // ends up connected to no-one.
        var lastStopAt: Date? = nil
        static let stopStartCooldown: TimeInterval = 0.6
        ...
        func start(language: String) async {
            ...
            // Wait out any unfinished teardown from the previous stop().
            if let ts = lastStopAt {
                let elapsed = Date().timeIntervalSince(ts)
                if elapsed < Self.stopStartCooldown {
                    let remaining = Self.stopStartCooldown - elapsed
                    TranscribeHelper.writeStderr(
                        "[start] cooldown: waiting \(Int(remaining * 1000))ms for previous teardown\n"
                    )
